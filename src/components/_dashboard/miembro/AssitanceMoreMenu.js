...
const ModalAssistance = ({ id_persona,id_miembro,open, handleOpen, handleClose,handleClick }) => {
    ...
    const formikAssistanceSantaCena = useFormik({
        initialValues: {
          fecha: '',
          hora: '',
          descripcion: '',
        ...
        validationSchema: Yup.object({
          ...
          id_miembro: Yup.string().required('Required')          
        }),
        onSubmit: async(values,{ resetForm }) => {           
          const dateFormat=new Date(values.fecha).toLocaleDateString('en-GB');
          const timeFormat=new Date(values.hora).toLocaleTimeString('en-GB').slice(0, -3);        
  
           const response= await axios.post('http://localhost:5000/api/miembros/asistenciasc', {fecha:dateFormat,
                                                                        hora:timeFormat,
                                                                        estado:values.estado,
                                                                        descripcion:values.descripcion,
                                                                        id_miembro:values.id_miembro});  
          if(response.status==200){
            resetForm();
            handleClick();
            handleClose();
        ...
        <Box sx={style}>
               <TabContext value={value}>
                                  ...
                                  scrollButtons>
                                       ...
                                       <Tab label="Asistencia Santa Cena" value="2" />                                
                                       </TabList>
                                   </Box>
    
                                   <TabPanel value="1"><Asistance formik={formikAssistance} handleClose={handleClose}/>                                   </TabPanel>
                                   <TabPanel value="2"><AssitanceSantaCena formik={formikAssistanceSantaCena} handleClose={handleClose}></AssitanceSantaCena></TabPanel>
